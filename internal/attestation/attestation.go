...
func HandleAttestation(logger *log.Logger, cfg *types.Config, platformName string) error {
...
func AttestationKeyRegistration(logger *log.Logger, registration types.Registration, attestationKey string, platform string) error {
	...
	// Retry mechanism: try up to 10 times
	maxRetries := 10
	...
	for attempt := 1; attempt <= maxRetries; attempt++ {
		...
		if err != nil {
			// Check if the error is network unreachable
			...
			// After all retries failed, return a regular error, not ErrNeedNet
			return fmt.Errorf("failed to register attestation key after %d attempts: %w", maxRetries, lastErr)
		}
		...
		if resp.StatusCode < 200 || resp.StatusCode >= 300 {
			lastErr = fmt.Errorf("registration failed with status code: %d", resp.StatusCode)
			logger.Info("Attestation registration attempt %d/%d failed with status %d", attempt, maxRetries, resp.StatusCode)
